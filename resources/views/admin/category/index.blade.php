                ...
                <div class="row">
                    <div class="col-lg-12 col-md-12 col-sm-12">
                        <h2><a href="javascript:void(0);" class="btn btn-xs btn-link btn-toggle-fullwidth"><i
                                    class="fa fa-arrow-left"></i></a> Categorys
                            <a class="btn btn-sm btn-outline-secondary" href="{{ route('category.create') }}">Create
                                Categorys</a>
                        </h2>
                        <ul class="breadcrumb ">
                                        ...
                                        @if ($categorys->count() > 0)
                                            ...
                                            @foreach ($categorys as $category)
                                                ...
                                                <tr>
                                                    ...
                                                    <td><img src="{{ $category->photo }}" alt="category image"
                                                    ...
                                                    <td>
                                                        <input type="checkbox" name="toogle" value="{{ $category->id }}"
                                                            data-toggle="switchbutton" {{ $category->status == 'active' ? 'checked' : '' }}
...
$('.dlBtn').click(function(e){
    ...
    <script>
        $('input[name=toogle]').change(function() {
            ...
            // ajax code
            $.ajax({
                url: "{{ route('category.status') }}", //this name in web.php route
                type: "POST",
                data: {
                    _token: '{{ csrf_token() }}',
                    mode: mode,
