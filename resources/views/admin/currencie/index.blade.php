                ...
                <div class="row">
                    <div class="col-lg-12 col-md-12 col-sm-12">
                        <h2><a href="javascript:void(0);" class="btn btn-xs btn-link btn-toggle-fullwidth"><i
                            ...
                            <a class="btn btn-sm btn-outline-secondary" href="{{ route('currencie.create') }}">Create currency</a>
                        </h2>
                                        ...
                                        @if ($currencys->count() > 0)
                                            ...
                                            @foreach ($currencys as $item)
                                               ...
                                               <td>{{ $item->code }}</td>
                                                    ...
                                                    <td>
                                                        <a href="{{ route('currencie.edit', $item->id) }}"
                                                        ...
                                                        <form class="float-left ml-2"
                                                            action="{{ route('currencie.destroy', $item->id) }}"
                                                            method="post">
                                                            @csrf
                                                            @method('delete')
...
@section('scripts')
    ...
    <script>
        $('input[name=toogle]').change(function() {
            ...
            // ajax code
            $.ajax({
                url: "{{ route('currencie.status') }}", //this name in web.php route
                type: "POST",
                data: {
                    _token: '{{ csrf_token() }}',
                    mode: mode,
