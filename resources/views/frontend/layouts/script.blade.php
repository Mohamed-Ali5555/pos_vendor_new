  ...
  <script>
      $(document).on('click', '.add_to_cart', function(e) {
        ...
        //   var product_qty = $(this).data('quantity');
        var product_qty = $(this).attr('data-quantity'); // Use .attr instead of .data to get the updated attribute
        var productQuantity = $("#update-cart-" + product_id).data('product-quantity'); // Get the product quantity from the hidden input
// alert(productQuantity);
          ...
          $.ajax({
              ...
              success: function(data) {
                  ...
                  if (data['status']) {
                      ...
                      $('.qty-text-counter').filter(function() {
                    return $(this).data('id') === product_id;
                }).val('1'); // Reset to initial value (e.g., '1')

                        // Reset the quantity input field
                $('#counter_qty').val('1'); // Reset to initial value
                $('.add_to_cart').filter(function() {
                    return $(this).data('product-id') === product_id;
                }).attr('data-quantity', '1'); // Optionally reset button data-quantity attribute
                  }
              }
          });
      });
  ...
  <script>
    ...
    //       $('#add_to_cart_' + id).attr('data-quantity', newVal);
  ...
  $(document).on('input', '#counter_qty', function() {
    
    var counter_qty = $(this).val();

    var productId = $(this).data('id');
    alert(counter_qty);
    console.log('Input value:', counter_qty); // Debug
    console.log('Product ID:', productId); // Debug

    $('.add_to_cart').filter(function() {
        return $(this).data('product-id') === productId;
    }).attr('data-quantity', counter_qty);
    // $('#add_to_cart_' + productId).attr('data-quantity', counter_qty);
