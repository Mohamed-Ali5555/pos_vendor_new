                                ...
                                <table class="table table-bordered mb-30">
                                    ...
                                    <tbody>
                                        @foreach (\Gloudemans\Shoppingcart\Facades\Cart::instance('shopping')->content() as $item)
                                            <tr>
                                                <td><img src="{{ $item->model->photo }}" alt="product"></td>
                                                <td>
                                                    <a href="{{ route('product.detail', $item->model->slug) }}">{{ $item->name }}
                                                </td>
                        ...
                        <form action="{{ route('checkout.store') }}">
                                ...
                                <table class="table mb-0">
                                    <tbody>
                                        ...
                                        <tr>
                                            <td>shipping</td>
                                            <td>
                                                ${{ \Illuminate\Support\Facades\Session::get('checkout')[0]['delivery_charge'] }}
                                            </td>
                                        </tr>
                                        ...
                                        @if (\Illuminate\Support\Facades\Session::has('coupon'))
                                            <tr>
                                                <td>coupon</td>
                                                <td>${{ number_format(\Illuminate\Support\Facades\Session::get('coupon')['value'], 2) }}
                                                </td>
                                            </tr>
                                        @endif
                                        <tr>
                                            <td>Total</td>
                                            {{-- // sub_total - minus coupon --}}
                                            <td>

                                                @if (\Illuminate\Support\Facades\Session::has('coupon') && \Illuminate\Support\Facades\Session::get('checkout'))
                                            <td>${{ number_format(
                                                (float) str_replace(',', '', \Illuminate\Support\Facades\Cart::subtotal()) +
                                                    \Illuminate\Support\Facades\Session::get('checkout')[0]['delivery_charge'] -
                                        ...
                                        @elseif(\Illuminate\Support\Facades\Session::get('checkout')[0]['delivery_charge'])
                                            <td>$
                                                {{ number_format((float) str_replace(',', '', \Gloudemans\Shoppingcart\Facades\Cart::subtotal()) + \Illuminate\Support\Facades\Session::get('checkout')[0]['delivery_charge']) }}
                                            </td>
                                        @else
                                            <td>$ {{ number_format(\Gloudemans\Shoppingcart\Facades\Cart::subtotal()) }}
                                            </td>
                                            @endif
                                            </td>
                                        </tr>
                            ...
                            <div class="checkout_pagination d-flex justify-content-end mt-3">
                                <a href="checkout-4.html" class="btn btn-primary mt-2 ml-2 d-none d-sm-inline-block">Go
                                    Back</a>
                                <button href="" class="btn btn-primary mt-2 ml-2">Confirm</button>
                            </div>
                        </form>
                    </div>
                </div>
